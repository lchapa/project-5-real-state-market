...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {        
		...
		require(_tokenOwner[tokenId] != to, "Address to approve is already the owner of the token");
        
        // TODO require the msg sender to be the owner of the contract (token instead?) or isApprovedForAll() to be true
		require(msg.sender == _tokenOwner[tokenId] || isApprovedForAll(_tokenOwner[tokenId], msg.sender), "Sender is NOT token owner nor approved for all");
    ...
    function _mint(address to, uint256 tokenId) virtual internal {
        ...
        require(to != address(0), "Invalid address of the minted token");
  		require(_tokenOwner[tokenId] == address(0), "Token already exist in contract and has owner");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) virtual internal {
        ...
        require(from == _tokenOwner[tokenId], "The from address is NOT the current owner of the token to transfer");
...
contract ERC721Metadata is ERC721Enumerable {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId));
		_tokenURIs[tokenId] = bytes(_baseTokenURI).length > 0 ? string(abi.encodePacked(_baseTokenURI, tokenId.toString())) : "";        
    }
}
